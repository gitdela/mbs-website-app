      ...
      <div className='w-full'>
        ...
        <div className='w-full'>
          <table className='text-black w-full md:w-[80%] mx-auto mb-6'>
            ...
            <thead className='bg-[#2D5992]/10'>
              <tr>
                <th
                  scope='col'
                  className='md:px-6 py-4 pl-2 text-left whitespace-nowrap text-white font-normal text-xs md:text-sm'
                >
                  Name
                </th>
                <th
                  scope='col'
                  className='md:px-6 py-4 text-right whitespace-nowrap text-white font-normal text-xs md:text-sm'
                >
                  Price (USD)
                </th>
                <th
                  scope='col'
                  className='md:px-6 py-4 text-right hidden md:table-cell whitespace-nowrap text-white font-normal text-xs md:text-sm'
                >
                  Last Price
                </th>
                <th
                  scope='col'
                  className='md:px-6 py-4 pe-2 text-right md:table-cell text-white font-normal text-xs md:text-sm'
                >
            ...
            <tbody className='md:divide-y divide-[#2D5992]/20 border-[#2D5992]'>
              {rates.length > 0 &&
                rates.map((value: any, index: any) => (
                  <tr key={index} className='cursor-pointer'>
                    <td className='px-2 md:px-6 py-4 whitespace-nowrap text-sm text-white flex w-fit items-center gap-1 md:gap-2'>
                      <Image
                        priority
                        src={value.image}
                        alt='crypto'
                      ...
                      <div className='flex flex-col items-start'>
                        ...
                        <span className='uppercase text-xs md:text-base font-light text-muted-foreground'>
                          {value.ticker}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className='px-2 ps-8 md:px-6 whitespace-nowrap text-right text-white'>
                      {Number(value.price_change_percentage_24h) > 0 ? (
                        <div className='bg-[#2caf88] max-md:w-20 w-20 py-2 md:px-4 text-xs ms-auto rounded text-center font-semibold'>
                          +
                          {Number(
                        ...
                        <div className='bg-red-500 max-md:w-20 w-20 py-2 md:px-4 rounded text-xs ms-auto text-center font-semibold'>
                          {Number(
                            value.price_change_percentage_24h
                          ).toLocaleString(undefined, {
                            minimumFractionDigits: 2,
